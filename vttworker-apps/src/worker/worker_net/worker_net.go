...
func (w WorkerNet) sendMessage(sockIdentity, cliIdentity, msgPrefix string, msgBodyTable common.CustomMap) {
	...
	if err == nil {
		...
		_,sndErr := w.dealer.SendMessage(sndMessage)
		if sndErr != nil {
...
func (w *WorkerNet) dealerZMQPollIn() {
	...
	totalCount += 1
	parts, recvErr := w.dealer.RecvMessageBytes(0)
	if recvErr != nil {
	...
	_logger.Info("sockIdentity: %s | cliIdentity: %s | msg: %s .", sockIdentity, cliIdentity, msg)
...
func (w *WorkerNet) init(routerAddr string, workerId string) {
	...
	w.dealer.SetRcvhwm(w.rcvhwmValue)
	w.dealer.SetLinger(0)
	w.dealer.SetIdentity(workerId)
	...
	_,err = w.dealer.SendMessage([]byte(STATUS_WORKER_KEEPALIVE)) // now send first alive msg
	if err != nil {
		_logger.Error("w.dealer send error: %v", err)
	}
	
	w.poller.Add(w.dealer, zmq.POLLIN)

	lastKeepAliveTimestamp = time.Now()
	preClock = time.Now() //common_api.getClocks()
	curClock = preClock
	bInitSocket = true
	fmt.Println("-------weihm:Init Socket finish-----")
...
func (w *WorkerNet) BackgroundTask() bool {
	if !bInitSocket || w.poller ==nil{
		return false
	}
	
    sockets, err := w.poller.Poll(500 * time.Millisecond)
    if err!= nil{
	...
	if curClock.Sub(lastKeepAliveTimestamp).Nanoseconds() >= KEEP_ALIVE_INTERNAL {
		if w.dealer != nil {
			w.dealer.SendMessage([]byte(STATUS_WORKER_KEEPALIVE))
